...
const getNextAllowedColors = {
  red: {
    red: false, green: true, yellow:true
  },
  yellow: {
    red: true, green: true, yellow:false
  },
  green: {
    red: true, green: false, yellow:true
  }
};

const addState  = (State, self) => {
...
class ControlledPlayground extends HTMLDivElement {
    ...
    renderControls() {
        ...
        return Object.keys(controlsState).map(controlState =>
            controlsState[controlState] ? html`<button class=${controlState}>${controlState}</button>` : html``);
    }
    render() {
        this.html`<div class="controls" ref=${this.refs.controls}>
                      ${this.renderControls()}
                  </div>
                  <div class="playground" style="background:${this.getState().play}; height:100px; width:100px">
